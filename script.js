...
var enPasRank = -1;
var enPasFile = -1;
var pawnNearby = false;

let emptyHash = parseHexString("0000000000000000");
...
function xorHash(arr, num) {
	return xorArrays(arr, parseHexString(hashes[num]));
}
...
function parseEnPas(str) {
	...
	let f = str.charCodeAt(0)-97;
	let r = str.charCodeAt(1)-49;
	if((f<0)||(f>7)||(r<0)||(r>7)){
		error = true;
		return;
	}
...
function hashSide(arr, str){
	if(str=="w")
		return xorHash(arr,780);
	else if(str != "b") {
		error = true;		
	}
	return arr;
...
function hashPieces(arr, str) {
	...
	for(let i = 0; i<8; i++) {
		...
		for(let j = 0; j<ranks[i].length; j++){
			switch(ranks[i][j]) {
				case "p":
					arr = xorHash(arr,64*0+8*rank+file);					
					if((enPasRank==2)&&(rank==3)&&(enPasFile>0)&&(file==enPasFile-1))
						pawnNearby = true;
					if((enPasRank==2)&&(rank==3)&&(enPasFile<7)&&(file==enPasFile+1))
						pawnNearby = true;	
					file++;
					break;					
				case "P":
					arr = xorHash(arr,64*1+8*rank+file);
					if((enPasRank==5)&&(rank==4)&&(enPasFile>0)&&(file==enPasFile-1))
						pawnNearby = true;
					if((enPasRank==5)&&(rank==4)&&(enPasFile<7)&&(file==enPasFile+1))
						pawnNearby = true;					
					file++;
					break;
				case "n":
					arr = xorHash(arr,64*2+8*rank+file);
					file++;
					break;					
				case "N":
					arr = xorHash(arr,64*3+8*rank+file);
					file++;
					break;
				case "b":
					arr = xorHash(arr,64*4+8*rank+file);
					file++;
					break;					
				case "B":
					arr = xorHash(arr,64*5+8*rank+file);
					file++;
					break;	
				case "r":
					arr = xorHash(arr,64*6+8*rank+file);
					file++;
					break;					
				case "R":
					arr = xorHash(arr,64*7+8*rank+file);
					file++;
					break;	
				case "q":
					arr = xorHash(arr,64*8+8*rank+file);
					file++;
					break;					
				case "Q":
					arr = xorHash(arr,64*9+8*rank+file);
					file++;
					break;	
				case "k":
					arr = xorHash(arr,64*10+8*rank+file);
					file++;
					break;					
				case "K":
					arr = xorHash(arr,64*11+8*rank+file);
					file++;
					break;	
				case "0":
				case "1":
...
function fenChange(){
	...
	if(pawnNearby)
	{
		hash = xorHash(hash,772+enPasFile);
	}
	hash = hashSide(hash, fen[1]);
	hash = hashCastle(hash, fen[2]);
	document.querySelector("#hash").innerHTML = error ? "Wrong position" : createHexString(hash);
}
